...
revigo.data$description <- wrap(revigo.data$description, 15)
...
p1 <- p1 + geom_label_repel( data = sig, aes(plot_X, plot_Y, label = description), 
                            colour = I(alpha("black", 0.85)), size = 2,
                            lineheight = 0.7, segment.color="black", segment.size=0.2,
                            min.segment.length=1, force=1);
